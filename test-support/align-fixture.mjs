...
export function makeAlignInput(overrides = {}) {
  return {
    ...
    intent: [
      ...
      {
        statement: "각 원본 업로드에는 하나의 복구 기록이 연결되고 그 기록은 대기, 진행, 완료, 취소 중 하나의 상태를 갖는다.",
        verify: "복구 데이터 모델과 상태 전이를 검사해 원본 업로드별 기록의 유일성과 허용된 상태를 확인한다.",
        by: "agent",
        decidedBy: "user",
        reason: "복구 정체성과 수명 주기를 합의하면 중복 재개와 서로 충돌하는 상태를 예방할 수 있다.",
      },
    ],
    exclusions: [
      "다른 사용자의 업로드 인계",
