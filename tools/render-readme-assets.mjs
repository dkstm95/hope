...
function diffAnalysis(snapshot) {
  ...
  const t = ko
    ? {
        ...
        before: "음수 길이는 반복을 끝내지 못해 메모리가 소진될 때까지 문자열을 늘릴 수 있었습니다.",
        ...
        impactOther: "변경이 두 반복문 안에 있어 수집하지 않은 호출자는 주요 설명에 영향을 주지 않습니다.",
        impactRun: "Hope가 테스트를 실행하지 않아 PR의 통과 결과는 아직 확인되지 않았습니다.",
        purpose: "음수 길이에서 비보안 ID 생성이 무한 반복되지 않게 합니다.",
        title: "음수 길이에서도 비보안 ID 생성이 바로 끝납니다.",
        reviewDone: "PR에 적힌 v3 테스트가 모두 끝나고 각 음수 길이 입력이 빈 문자열을 반환합니다.",
        reviewEffect: "코드와 테스트에는 의도가 드러나지만 v3 테스트 결과는 확인되지 않았습니다.",
        reviewExplanation: "PR에는 11개 테스트가 통과했다고 적혀 있지만 Hope는 명령을 실행하지 않았습니다.",
        reviewNext: "v3 브랜치에서 PR에 적힌 uvu 명령을 실행하세요.",
        reviewTitle: "v3 테스트 결과 확인",
        why: "0과 양수의 동작은 유지하고 음수의 무한 반복만 막습니다.",
      }
    : {
        after: "A negative size skips the loop and returns an empty string.",
        before: "A negative size could keep the loop running and grow the string until memory ran out.",
        ...
        impactOther: "Unchecked callers do not affect the main explanation because the change stays inside the two loops.",
        impactRun: "Hope did not run the tests, so the PR's passing result is not yet confirmed.",
        purpose: "Prevent negative sizes from trapping non-secure ID generation in a loop.",
        title: "Non-secure ID generation now stops for negative sizes.",
        reviewDone: "The PR's v3 test command finishes, and every negative-size case returns an empty string.",
        reviewEffect: "The code and tests show the intent, but the v3 result is not confirmed.",
        reviewExplanation: "The PR reports 11 passing tests; Hope did not run the command.",
        reviewNext: "Run the PR's uvu command on the v3 branch.",
        reviewTitle: "Confirm the v3 test result",
        why: "Zero and positive sizes behave as before; only the negative-size loop stops.",
      };
  return {
    behavior: {
      steps: [
