...
분석은 변경을 만든 대화를 상속하지 않고 정확한 검토 요청과 PR만 전달받은 새
컨텍스트에서 진행합니다.
...
>
> 모든 리뷰어는 별도의 모델 호출을 사용합니다. 여러 리뷰어를 병렬로 실행하면 시간을
...
<details>
<summary><strong>Polish</strong> — 확정한 동작을 지키며 완성된 작업을 개선합니다</summary>

Polish는 이름을 지정한 완성된 결과나 현재 저장소 변경분을 검토하고, 범위를 제한한
개선 작업을 한 번 적용합니다.

서로 독립된 정리 검토자가 가치 있는 개선만 찾습니다. 코드에서는 기존 도우미 재사용,
단순성, 효율, 추상화 수준을 확인합니다. 새 마무리 작업자가 근거를 판정하고 추측에
가깝거나 위험한 제안을 제외한 뒤, 함께 적용할 수 있는 변경만 반영하고 검증합니다.

Polish는 정리 작업을 버그 탐색, 기능 개발, 마이그레이션, 광범위한 유지보수와
분리합니다. 이런 문제가 보이면 리팩터링에 숨기지 않고 따로 보고합니다.
...
전달 과정의 낭비, 불분명한 소유권처럼 프로젝트에 중요한 유지보수 위험을
살펴봅니다.

이 목록은 검토 방향을 제시할 뿐이며 Sweep이 모든 영역에서 문제를 찾아내야 하는
것은 아닙니다.

불완전한 근거는 문제가 없는 것처럼 표시하지 않습니다.

프로젝트를 여러 배치로 나눌 때는 각 검사자가 이전 결론을 상속하지 않은 새
컨텍스트에서 겹치지 않는 범위를 맡습니다.
