...
그럴듯한 결과물에도 중요한 문제, 근거 없는 주장, 불필요한 복잡성이 숨어 있을 수 있습니다.

Toxic Review는 대상과 위험도에 맞는 관점만 선택합니다.

근거가 연결된 지적을 우선순위가 있는 하나의 리뷰로 정리합니다.
...
Diff는 하나의 정확한 GitHub PR 스냅샷을 기준으로 코드보다 동작을 먼저 설명하고 중요한 주장에 근거를 연결합니다.
...
문장이 매끄러워도 사실을 잃거나 불확실성을 없애고 사용자의 말투를 지우면 잘못된 글입니다.
...
- 낡은 비유와 상투어를 피합니다.
- 같은 뜻이면 짧고 익숙한 낱말을 고릅니다.
- 의미를 더하지 않는 말은 덜어냅니다.
- 행위자와 행동이 분명해진다면 능동태를 씁니다.
- 가능하면 전문용어 대신 일상어를 사용합니다.
- 규칙이 글을 부정확하거나 불명확하거나 부자연스럽거나 불필요하게
  가혹하게 만든다면 그 규칙을 따르지 않습니다.

Hope는 여기에 결론을 먼저 말하고, 한 문장에 하나의 중심 생각만
담으며, 의미, 사실, 불확실성, 인용, 숫자, 말투를 보존하는 기준을
더합니다.

Hope는 의미, 가독성, 호흡이 좋아진다면 산문 한 문단에 한 문장을
사용합니다.

Markdown과 일반 텍스트에서는 빈 줄 하나로 문단을 구분하고, 다른
형식에서는 해당 형식의 문단 구조를 사용합니다.

Write는 의미 구조를 선택하고, 렌더러는 화면 간격, 서체, 스타일을
결정합니다.

문장을 나누면 의미, 흐름, 말투가 나빠지거나 대상 형식에 맞지 않는다면
관련 문장을 한 문단에 둡니다.

해당 언어로 처음부터 쓴 글처럼 표현하며 다른 언어의 어순과 관용구를
그대로 옮기지 않습니다.

Write는 글을 작성하고, 고치고, 검토합니다.
